...
const MANUAL_COMMIT_DATA = {
  ...
  "2026-03-01": { count: 5, level: 2 },
  "2026-03-02": { count: 2, level: 1 },
  "2026-03-03": { count: 1, level: 1 },
  "2026-03-04": { count: 3, level: 2 },
  "2026-03-05": { count: 17, level: 4 },
  "2026-03-06": { count: 5, level: 2 },
  "2026-03-07": { count: 1, level: 1 },
  "2026-03-09": { count: 1, level: 1 },
  "2026-03-10": { count: 2, level: 1 },
  "2026-03-11": { count: 1, level: 1 },
  "2026-03-12": { count: 1, level: 1 },
  "2026-03-13": { count: 1, level: 1 },
  "2026-03-14": { count: 1, level: 1 },
  "2026-03-15": { count: 2, level: 1 },
  "2026-03-16": { count: 1, level: 1 },
  "2026-03-17": { count: 11, level: 4 },
  "2026-03-18": { count: 6, level: 3 },
  "2026-03-19": { count: 1, level: 1 },
...
const GitHubContributionGraph = () => {
  ...
  useEffect(() => {
    ...
    if (activeDates.length > 0) {
      ...
      for (let i = 1; i < activeDates.length; i++) {
        const diffDays =
          (new Date(activeDates[i]) - new Date(activeDates[i - 1])) /
          (1000 * 60 * 60 * 24);
        if (diffDays === 1) {
          tempStreak++;
        } else {
      ...
      if ((todayReference - lastCommitDate) / (1000 * 60 * 60 * 24) <= 1) {
        ...
        while (pointer >= 0) {
          if (pointer === activeDates.length - 1) {
            ...
            const diff =
              (new Date(activeDates[pointer + 1]) -
                new Date(activeDates[pointer])) /
              (1000 * 60 * 60 * 24);
            if (diff === 1) {
              currentStreakCount++;
              currentStreakStart = activeDates[pointer];
  ...
  const getColor = (level) =>
    ["#16161e", "#053629", "#076b4b", "#0eb17a", "#00ffa2"][level] || "#16161e";
    ...
    const measure = () => {
      ...
      const fitSize = (text, targetW, startSize = 200) => {
        ...
        while (size > 8) {
          ...
          if (ctx.measureText(text).width * 0.93 <= targetW) return size;
          size--;
        }
        return size;
      };
    ...
    <div className="bg-black text-white min-h-screen font-sans selection:bg-white selection:text-black overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-[100] border-b border-zinc-900 bg-black/80 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between">
          ...
          >
            <motion.a
              href="https://www.linkedin.com/in/riteshpatel1884"
              target="_blank"
